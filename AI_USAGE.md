...
要求按照课程文件规定的 A3 分工和执行顺序，在现有仓库中完成公共任务模型。
...
- 在 A1/A2/B1/B2 和 B3 文件全部进入 `main` 后，重新读取四个服务的请求、响应、专有 Schema、产物、校验脚本及公共文档。
- 使用临时依赖运行四个 `validate.py`，全部完整通过；同时验证公共 Schema、八个公共样例和仓库内 19 个 JSON 文件。
- 根据实际结果清理文档中的过期状态，包括“等待 A1/A2”“缺少 jsonschema 未运行”和已可从 Git 历史解析的提交 SHA。
- 没有修改四个服务的接口样例、服务专有 Schema、产物或校验逻辑；未把离线 fixture 验证描述成真实服务联调。
